...
class BackgroundCountdown extends BaseCountdown {
    ...
    private RectF mDayBgBorderRectF, mHourBgBorderRectF, mMinuteBgBorderRectF, mSecondBgBorderRectF;
    ...
    public void initStyleAttr(Context context, TypedArray ta) {
        ...
        mTimeBgDivisionLineSize = ta.getDimension(R.styleable.CountdownView_timeBgDivisionLineSize, DisplayUtil.dp2px(context, DEFAULT_TIME_BG_DIVISION_LINE_SIZE));
        mTimeBgSize = ta.getDimension(R.styleable.CountdownView_timeBgSize, 0);
        mDefSetTimeBgSize = mTimeBgSize;
        mTimeBgBorderSize = ta.getDimension(R.styleable.CountdownView_timeBgBorderSize, DisplayUtil.dp2px(context, DEFAULT_TIME_BG_BORDER_SIZE));
    ...
    protected void initTimeTextBaseInfo() {
        ...
        if (mDefSetTimeBgSize == 0 || mTimeBgSize < mTimeTextWidth) {
            mTimeBgSize = mTimeTextWidth + (DisplayUtil.dp2px(mContext, 2) * 4);
        }
    }
    ...
    private void initHasBackgroundTextBaseY(RectF rectF) {
        ...
        mTimeBgDivisionLineYPos = rectF.centerY() + (mTimeBgDivisionLineSize == DisplayUtil.dp2px(mContext, DEFAULT_TIME_BG_DIVISION_LINE_SIZE) ? mTimeBgDivisionLineSize : mTimeBgDivisionLineSize / 2);
    }
    ...
    public int getAllContentWidth() {
        ...
        if (isShowDay) {
            if (isDayLargeNinetyNine) {
                ...
                mDayTimeBgWidth = rect.width() + (DisplayUtil.dp2px(mContext, 2) * 4);
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (isShowDay) {
            ...
            // draw day text
            canvas.drawText(DisplayUtil.formatNum(mDay), mDayBgRectF.centerX(), mTimeTextBaseY, mTimeTextPaint);
        ...
        if (isShowHour) {
            ...
            // draw hour text
            canvas.drawText(DisplayUtil.formatNum(mHour), mHourBgRectF.centerX(), mTimeTextBaseY, mTimeTextPaint);
        ...
        if (isShowMinute) {
            ...
            // draw minute text
            canvas.drawText(DisplayUtil.formatNum(mMinute), mMinuteBgRectF.centerX(), mTimeTextBaseY, mTimeTextPaint);
        ...
        if (isShowSecond) {
            ...
            // draw second text
            canvas.drawText(DisplayUtil.formatNum(mSecond), mSecondBgRectF.centerX(), mTimeTextBaseY, mTimeTextPaint);
            if (mSuffixSecondTextWidth > 0) {
                // draw second suffix
    ...
    public void setTimeBgSize(float size) {
        mTimeBgSize = DisplayUtil.dp2px(mContext, size);
    }
    ...
    public void setTimeBgRadius(float radius) {
        mTimeBgRadius = DisplayUtil.dp2px(mContext, radius);
    }
    ...
    public void setTimeBgDivisionLineSize(float size) {
        mTimeBgDivisionLineSize = DisplayUtil.dp2px(mContext, size);
        if (null != mTimeBgDivisionLinePaint) {
            mTimeBgDivisionLinePaint.setStrokeWidth(mTimeBgDivisionLineSize);
        }
    }
    ...
    public void setTimeBgBorderSize(float size) {
        mTimeBgBorderSize = DisplayUtil.dp2px(mContext, size);
    ...
    public void setTimeBgBorderRadius(float size) {
        mTimeBgBorderRadius = DisplayUtil.dp2px(mContext, size);
    }
