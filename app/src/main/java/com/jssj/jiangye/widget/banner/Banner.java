package com.jssj.jiangye.widget.banner;
...
import com.jssj.jiangye.widget.banner.loader.ImageLoaderInterface;
import com.jssj.jiangye.widget.banner.loader.OnBannerClickListener;
...
public class Banner extends FrameLayout implements OnPageChangeListener {
    ...
    private List<SlideShowData> imageUrls;
  ...
  /*  public Banner setImages(List<?> imageUrls) {
    ...
    public Banner setImages(List<SlideShowData> imageUrls) {
        this.imageUrls = imageUrls;
        this.count = imageUrls.size();
        return this;
    }

    public void update(List<SlideShowData> imageUrls, List<String> titles) {
    ...
    public void update(List<SlideShowData> imageUrls) {
        this.imageUrls.clear();
        this.imageUrls.addAll(imageUrls);
        this.count = this.imageUrls.size();
        start();
    ...
    private void setImageList(List<SlideShowData> imagesUrl) {
        ...
        for (int i = 0; i <= count + 1; i++) {
            ...
            Object url = null;
            if (i == 0) {
                url = imagesUrl.get(count - 1).getAdverImageUrl();
            } else if (i == count + 1) {
                url = imagesUrl.get(0).getAdverImageUrl();
            } else {
                url = imagesUrl.get(i - 1).getAdverImageUrl();
            }
        ...
        public Object instantiateItem(ViewGroup container, final int position) {
            ...
            if (listener != null) {
                view.setOnClickListener(new OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        listener.OnBannerClick(imageUrls.get(toRealPosition(position)));
                    }
                });
            }
            return view;
