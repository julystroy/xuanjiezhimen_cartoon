package com.jssj.jiangye.widget.countdown;
...
class BaseCountdown {
    ...
    public void initStyleAttr(Context context, TypedArray ta) {
        ...
        mTimeTextSize = ta.getDimension(R.styleable.CountdownView_timeTextSize, DisplayUtil.sp2px(mContext, 12));
        ...
        mSuffixTextSize = ta.getDimension(R.styleable.CountdownView_suffixTextSize, DisplayUtil.sp2px(mContext, 12));
    ...
    private void initSuffixMargin() {
        int defSuffixLRMargin = DisplayUtil.dp2px(mContext, DEFAULT_SUFFIX_LR_MARGIN);
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (isShowDay) {
            // draw day text
            canvas.drawText(DisplayUtil.formatNum(mDay), mLeftPaddingSize + mDayTimeTextWidth / 2, mTimeTextBaseline, mTimeTextPaint);
        ...
        if (isShowHour) {
            // draw hour text
            canvas.drawText(DisplayUtil.formatNum(mHour), mHourLeft + mTimeTextWidth / 2, mTimeTextBaseline, mTimeTextPaint);
        ...
        if (isShowMinute) {
            // draw minute text
            canvas.drawText(DisplayUtil.formatNum(mMinute), mMinuteLeft + mTimeTextWidth / 2 , mTimeTextBaseline, mTimeTextPaint);
        ...
        if (isShowSecond) {
            // draw second text
            canvas.drawText(DisplayUtil.formatNum(mSecond), mSecondLeft + mTimeTextWidth / 2, mTimeTextBaseline, mTimeTextPaint);
            if (mSuffixSecondTextWidth > 0) {
                // draw second suffix
            ...
            if (isShowMillisecond) {
                ...
                canvas.drawText(DisplayUtil.formatMillisecond(mMillisecond), mMillisecondLeft + mTimeTextWidth / 2, mTimeTextBaseline, mTimeTextPaint);
    ...
    public boolean refTimeShow(boolean isShowDay, boolean isShowHour, boolean  isShowMinute, boolean isShowSecond) {
        if (!isShowSecond) isShowMillisecond = false;
    ...
    public void setTimes(int day, int hour, int minute, int second) {
        mDay = day;
        mHour = hour;
        mMinute = minute;
        mSecond = second;
    ...
    public void setTimeTextSize(float textSize) {
        if (textSize > 0) {
            mTimeTextSize = DisplayUtil.sp2px(mContext, textSize);
            mTimeTextPaint.setTextSize(mTimeTextSize);
        }
    }
    ...
    public void setSuffixTextSize(float textSize) {
        if (textSize > 0) {
            mSuffixTextSize = DisplayUtil.sp2px(mContext, textSize);
            mSuffixTextPaint.setTextSize(mSuffixTextSize);
        }
    }
    ...
    public void setSuffixLRMargin(float suffixLRMargin) {
        mSuffixLRMargin = DisplayUtil.dp2px(mContext, suffixLRMargin);
    ...
    public boolean setSuffixMargin(Float suffixDayMarginL, Float suffixDayMarginR,
        ...
        if (null != suffixDayMarginL) {
            mSuffixDayLeftMargin = DisplayUtil.dp2px(mContext, suffixDayMarginL);
            isRef = true;
        }
        if (null != suffixDayMarginR) {
            mSuffixDayRightMargin = DisplayUtil.dp2px(mContext, suffixDayMarginR);
            isRef = true;
        }

        if (null != suffixHourMarginL) {
            mSuffixHourLeftMargin = DisplayUtil.dp2px(mContext, suffixHourMarginL);
            isRef = true;
        }
        if (null != suffixHourMarginR) {
            mSuffixHourRightMargin = DisplayUtil.dp2px(mContext, suffixHourMarginR);
            isRef = true;
        }

        if (null != suffixMinuteMarginL) {
            mSuffixMinuteLeftMargin = DisplayUtil.dp2px(mContext, suffixMinuteMarginL);
            isRef = true;
        }
        if (null != suffixMinuteMarginR) {
            mSuffixMinuteRightMargin = DisplayUtil.dp2px(mContext, suffixMinuteMarginR);
            isRef = true;
        }

        if (null != suffixSecondMarginL) {
            mSuffixSecondLeftMargin = DisplayUtil.dp2px(mContext, suffixSecondMarginL);
            isRef = true;
        }
        if (null != suffixSecondMarginR) {
            mSuffixSecondRightMargin = DisplayUtil.dp2px(mContext, suffixSecondMarginR);
            isRef = true;
        }

        if (null != suffixMillisecondMarginL) {
            mSuffixMillisecondLeftMargin = DisplayUtil.dp2px(mContext, suffixMillisecondMarginL);
            isRef = true;
        }
